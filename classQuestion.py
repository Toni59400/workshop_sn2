...
class Question():
    ...
    def update(self):
        ...
        if self.timerNext > 0:
            self.screen.blit(self.img_btn[1], (60*self.pixel[0], 140*self.pixel[1]))
            self.timerNext -= 1
            if self.timerNext == 0:
                self.game.eventSuivant()
        else:
            if self.game.phase == "question" : 
                ...
                self.screen.blit(self.img, (0,0))
                self.screen.blit(self.img_btn[0], (60*self.pixel[0], 140*self.pixel[1]))
                cpt =0
